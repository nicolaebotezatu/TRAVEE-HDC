...
class HapticAdapter(Thread):
    def __init__(self, debugLevel=logging.DEBUG):
        ...
        ### Signals ###
        self.signalGotData = "hap1"
        self.signalGotStatus = "hap2"
    ...
    def run(self):
        ...
        # Start the HapticBLEBridge app on local machine
        if self.ip == "localhost" or self.ip == "127.0.0.1":
            os.system("start cmd /k bridgeApps\HapticBLEBridge.exe "
                    + self.ip
                    + " "
                    + str(self.port))
    ...
    def messageParser(self, msg):
        ...
        if "C=" in msg:
            dispatcher.send(self.signalGotStatus, self, type="HAPTIC", id=msg[2:19], data="connected")
            # if self.imuList.index()
        if "D=" in msg:
            dispatcher.send(self.signalGotStatus, self, type="HAPTIC", id=msg[2:19], data="disconnected")

    ### "SLOTS" ###
    def handlerPlayPattern(self, pattern):
        self.log.debug(f"Playing haptic pattern no. {pattern}")
